...
function App() {
  ...
  const handleLogin = (e) => {
    ...
    console.log(PATH);
    axios
      .post(`${URL}/v2/admin/signin`, account)
      .then((res) => {
        const { token, expired } = res.data;
        console.log(token, expired);
        document.cookie = `hexToken=${token}; expires=${new Date(expired)}`;
        axios
          .get(`${URL}/v2/api/${PATH}/admin/products`)
    ...
    <>
      {isAuth ? (
        <div className="container py-5">
          <div className="row">
            <div className="col-6">
              <button onClick={checkUserLogin} className="btn btn-success" type="button">檢查使用者是否登入</button>
              <h2>產品列表</h2>
